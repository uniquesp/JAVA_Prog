...
class Prog13 extends JFrame implements ActionListener
{
	...
	Prog13(String title)
	{
			...
			try
		{
			...
			rs = stm.executeQuery("select * from Student order by S_id");
			rs.first();
			display();		}
		catch(Exception e)
		{
			...
			try
		{
			if(e.getSource() == Cadd)
			{
				sql = "insert into Course values("+c_id.getText()+",'"+c_name.getText()+"','"+instructor.getText()+"')";
			...
			if(e.getSource() == CviewAll)
			{
				rs = stmt.executeQuery("Select * from SC");
				while(rs.next())
				{
					t6.setText(rs.getInt(s_id));
					t7.setText(rs.getString(s_course));
				}
			}
		
			if(e.getSource() == Sadd)
			{
				sql = "insert into Student values("+s_id.getText()+",'"+s_name.getText()+")";
				prstm = cn.prepareStatement(sql);
				prstm.execute();
				prstm.close();
				JOptionPane.showMessageDialog(null, "Record saved Successfully !!!");
			...
			{
				sql = "delete from Student where s_id = "+s_id.getText();
				prstm = cn.prepareStatement(sql);
				prstm.execute();
				prstm.close();
			...
			{
				int r = t4.getText();
				rs = stmt.executeQuery("Select * from Student where roll no = "+r);
				if(rs.next())
				{
					t4.setText(rs.getInt(s_id));
					t5.setText(rs.getString(s_name))
